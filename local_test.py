...
def get_db_connection():
    ...
    c = conn.cursor()
    c.execute('CREATE TABLE IF NOT EXISTS seen_jobs (job_id TEXT PRIMARY KEY, title TEXT, company TEXT, link TEXT, first_seen TEXT)')
    conn.commit()
    return conn, c
...
def fetch_workday_jobs(company):
    ...
    while True:
        ...
        if response.status_code != 200:
            print(f"Warning: {company['name']} returned {response.status_code}, stopping.")
            break
...
def fetch_successfactors_jobs(company, max_pages=10):
    ...
    while True:
        if pages_fetched >= max_pages:
            print(f"Reached max pages limit ({max_pages}) for {company['name']}.")
            break
...
def fetch_greenhouse_jobs(company):
    print(f"Fetching Greenhouse: {company['name']}...")
    url = f"https://boards-api.greenhouse.io/v1/boards/{company['board_token']}/jobs"
    try:
        response = requests.get(url, timeout=10) 
        
        if response.status_code != 200:
            print(f"Warning: {company['name']} returned {response.status_code}")
            return []
        ...
        print(f"{company['name']}: fetched {len(normalized_jobs)} jobs")
        return normalized_jobs
    except Exception as e:
        print(f"Error for Greenhouse {company['name']}: {e}")
        return []
...
def main():
    ...
    try:
        ...
        print(f"Error: Failed to load companies.json: {e}")
        conn.close()
        return
